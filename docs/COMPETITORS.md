...
Purser is **not** unique for multi-CLI orchestration, git worktrees, a desktop console, voice dictation, or a phone client. Those exist. This page is the honest matrix. Sources are linked. If a cell is blank, we did not verify it.
...
| --- | --- | --- | --- |
| **Purser** | Local companion: web UI + runner on loopback. User’s CLIs and keys. | Yes (several adapters, session worktrees, `/phone` via relay). | **Yes, shipped.** See [METERING.md](METERING.md), runner `budget.ts`, `audit.ts`. |
